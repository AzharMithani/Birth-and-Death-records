...
def convert(filename, sheetname,output_filename):
    data_xls = pd.read_excel(filename, sheetname,header = 4, dtype = {'Date Of Event':object}, index_col=None)
    data_xls = data_xls[4:]
    data_xls['Sr. No.'] = data_xls['Sr. No.'] - 4
    data_xls['Registration No'] = data_xls['Registration No'].str.strip()
    new_dates = []
